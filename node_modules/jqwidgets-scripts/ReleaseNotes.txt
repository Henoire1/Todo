jQWidgets v24.0.0 Release, September-17-2025

What's New

Tailwind Theme – Added a new Tailwind CSS-based theme for a modern, utility-first styling approach.
React 19.1 Support – Updated all React components to be fully compatible with React 19.1, ensuring smooth integration and updated hooks support.

What's Fixed

jqxGrid Resizing – Fixed an issue where the grid did not resize correctly when its container size changed, ensuring responsive behavior.
jqxGrid Checkbox Selection – Corrected checkbox selection behavior when filters are applied, so selected rows remain consistent after filtering.
jqxGrid DropdownList & CheckedList Filters – Resolved rendering issues for dropdown and checked list filters to ensure proper display and interaction.
jqxGrid Scroll Position after Sorting is correctly working now.
jqxExpander Header Position – Fixed a problem where dynamically updating the headerPosition property did not refresh the UI correctly.
jqxDateTimeInput Parsing – Fixed copy-paste issues with date input where different formats were not parsed correctly, improving cross-format support.
jqxButton Disabled State – Corrected dynamic updates to the disabled state of buttons to ensure proper styling and behavior when toggled programmatically.
jqxButtonGroup Disabled State – Fixed an issue where disabling button groups dynamically did not correctly disable all buttons in the group.
General UI Stability Improvements – Minor bug fixes and performance improvements across multiple components to ensure smoother rendering and interaction.

jQWidgets v23.1.0 Release, June-03-2025
...
- Fixed an issue in jqxGrid about the columns chooser and filter dropdownlist rendering when row details are enabled.
- Fixed an issue in jqxGrid about the autoresizecolumns method. 

jQWidgets v23.0.0 Release, May-26-2025

What's New

- Side Panel in jqxGrid
The new Side Panel feature in jqxGrid allows developers to add custom HTML elements directly into side panels positioned next to the grid. Whether you want to include filters, charts, summaries, help text, or interactive widgets, the side panel supports any type of content. This powerful new capability enables more contextual and productive UI layouts, without disrupting the core grid view.
...
- jqxTree – disableCheckbox Method
...
- Accessibility: WAI-ARIA in jqxGrid
Updated jqxGrid to improve WAI-ARIA compliance, ensuring better support for screen readers and improved accessibility in line with modern standards.
...
Fixed an issue in jqxInput about escaping html tags" - make this more professional
